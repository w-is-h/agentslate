...
- **Memory**: one page per project, subpages beneath it — the sections the project needs, typically purpose, environment, rules, catches, results, tidbits (dated, appended freely at any time). Write as things land mid-session — a catch goes in while it's fresh; session end only sweeps what slipped. Session start loads only this project's page — when you switch folders or need another repo's context, fetch its page with `memory_get`. A new page is created with me. Any memory work beyond a one-line edit loads the `slate-memory` skill.
