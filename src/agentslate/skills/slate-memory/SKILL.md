...
- **Environment** — where the project sits in the world: the machine it runs on, the port, the service unit, where the database lives, what stands in front of it (VPN, proxy, firewall), the external services it talks to. One test, applied literally: could this fact only be discovered by probing a machine, never by reading the repo? Config values, launch recipes, and code architecture are in the repo and stay out. A line here may repeat the project's row on `machines/<host>` — the project's view here, the machine's view there; that duplication is wanted.
