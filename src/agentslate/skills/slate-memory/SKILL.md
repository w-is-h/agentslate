...
- **Keep what future work needs and accessible artifacts do not carry.** A project may have code, other artifacts, or exist only as an idea. Preserve its general context in memory and link accessible artifacts — code, documents, canvases, images, graphs — for the details they hold. If no such artifacts exist, keep the relevant details in memory too. An artifact counts only if we can actually access it; knowing that something exists elsewhere is not enough.

The two jobs are the invariant; the sections are not. What follows is a suggested vocabulary — the shapes most projects need. A project keeps the sections that serve it and skips the rest, and a project type that needs a section not listed here adds it — held to the same tests: it serves one of the two jobs and is never a log.

- **Purpose and context** — what the project is, who is involved, whom it serves and how it works. Briefly repeating a README or other project description is fine — fast orientation beats deduplication.
...
Out: details already recoverable from accessible project artifacts, apart from the brief context needed for orientation. For code projects, these usually include layout, config values, module behavior, launch recipes, the current commit — a code change must never require a memory change. Out: status and progress already held by an accessible repo or task manager; a handover for a thread that continues next session goes on a canvas on the project's board, written only when the user asks for it or wraps the session naming the continuation, never by default. Out: history (the conversation archive and git hold it), general knowledge. A line that stopped being true is deleted.
